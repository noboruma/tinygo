...
func GetCachedGoroot(config *compileopts.Config) (string, error) {
	...
	overrides := pathsToOverride(config.GoMinorVersion, needsSyscallPackage(config.BuildTags()), needsNetTLSPackages(config.BuildTags()))
...
// needsNetTLSPackages returns whether the net/ net/http crypto/tls package should be overridden
// with the TinyGo version. This is the case on some targets.
